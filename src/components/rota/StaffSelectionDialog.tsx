  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[48rem] max-h-[90vh] flex flex-col">
        ...
        {/* Two-column layout: Staff list on left, Selection panel on right */}
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {/* Right column: Selection panel with Shift Options - Always visible */}
          <div className="md:col-span-1">
            <div className="sticky top-0 p-4 rounded-lg border bg-muted/30 space-y-4 h-fit">
              {/* Selected Staff Summary or Placeholder */}
              {!selectedStaffId && !isExternalTemp ? (
                <div className="text-center py-6">
                  <div className="h-12 w-12 rounded-full bg-muted flex items-center justify-center mx-auto mb-3">
                    <User className="h-6 w-6 text-muted-foreground" />
                  </div>
                  <p className="text-sm text-muted-foreground">Select a staff member</p>
                  <p className="text-xs text-muted-foreground mt-1">or add temp/locum staff</p>
                </div>
              ) : (
                <>
                  {/* Selected Staff Info */}
                  {selectedStaffId && !isExternalTemp && (() => {
                    const staff = filteredStaff.find(s => s.id === selectedStaffId);
                    ...
                    return (
                      <div className="pb-3 border-b">
                        ...
                        {staff.job_title_name && (
                          <span className={cn("text-xs px-2 py-0.5 rounded border inline-block mt-1", getJobTitleColors(staff.job_title_name))}>
                            {staff.job_title_name}
                          </span>
                        )}
                        <p className="text-sm text-muted-foreground mt-1">{hoursDisplay} scheduled</p>
                      </div>
                    );
                  })()}
                  ...
                  {isExternalTemp && (
                    <div className="pb-3 border-b">
                      ...
                      <p className="font-medium">{externalTempName || "External Temp Staff"}</p>
                      <span className="text-xs px-2 py-0.5 rounded border inline-block mt-1 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-800">
                        Temp/Locum
                      </span>
                    </div>
                  )}
                  ...
                  {/* Shift Options */}
                  {(showMakeFullDayOption || showCustomTimeOption) && (
