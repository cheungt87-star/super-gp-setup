...
CREATE OR REPLACE FUNCTION public.get_organisation_users(p_organisation_id uuid)
 RETURNS TABLE(id uuid, email text, first_name text, last_name text, phone text, job_title_id uuid, job_title_name text, primary_site_id uuid, site_name text, role text, is_active boolean, registration_completed boolean, contracted_hours numeric, working_days jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
